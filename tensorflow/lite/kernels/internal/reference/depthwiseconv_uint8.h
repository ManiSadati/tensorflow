...
struct DepthwiseConvBasicKernel {
  static inline void Run(
    ...
    // -------------------- Fault Injection (FI) --------------------
    // Convention for DepthwiseConvBasic output (NHWC, batch fixed to 0):
    //   c = output_channel (output_depth)
    //   x = out_x          (output_width)
    //   y = out_y          (output_height)
    // locations.txt lines: "c x y bit" using the convention above.
    FaultInjection FI;
    FI.init("DepthwiseConvBasic");
    FI.save_profile(/*c_dim=*/output_depth, /*x_dim=*/output_width,
                    /*y_dim=*/output_height, /*numOps=*/cnt);

    if (FI.isFaultyLayer()) {
      for (const auto& p : FI.injectLocations) {
        const int c = p.first.first;          // output_channel
    ...
    if (FI.isLoggedLayer()) {
      ...
      // Write values in loop order: c -> x -> y (batch=0).
